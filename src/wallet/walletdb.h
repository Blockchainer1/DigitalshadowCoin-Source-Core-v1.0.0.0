...
public:
    ...
    bool WriteZDSHASeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDSHASeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDSHASeed_deprecated(uint256& seed);
    bool EraseZDSHASeed();
    bool EraseZDSHASeed_deprecated();

    bool WriteZDSHACount(const uint32_t& nCount);
    bool ReadZDSHACount(uint32_t& nCount);
