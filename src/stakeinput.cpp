...
// The zDSHA block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zDSHA is a hash of the serial
...
{
    //Create an output returning the zDSHA that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZDSHAOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zDSHA output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zDSHA", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZDSHAOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zDSHA output", __func__);
...
{
    CzDSHATracker* zdshaTracker = pwallet->zdshaTracker.get();
    CMintMeta meta;
    if (!zdshaTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zdshaTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a DSHA stake is the outpoint
