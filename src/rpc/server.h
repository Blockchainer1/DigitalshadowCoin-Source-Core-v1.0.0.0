...
/**
 * DigitalShadow RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZdshaSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzdshaseed(const UniValue& params, bool fHelp);
extern UniValue getzdshaseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzdsha(const UniValue& params, bool fHelp);
extern UniValue dzdshastate(const UniValue& params, bool fHelp);
