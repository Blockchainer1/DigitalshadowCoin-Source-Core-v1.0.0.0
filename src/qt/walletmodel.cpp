...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered digitalshadow address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 DSHA.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal digitalshadow:URI (digitalshadow:XyZ...?message=example)
            {
...
static void NotifyzDSHAReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzDSHAReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzDSHAReset.connect(boost::bind(NotifyzDSHAReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzDSHAReset.disconnect(boost::bind(NotifyzDSHAReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zdshaTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
