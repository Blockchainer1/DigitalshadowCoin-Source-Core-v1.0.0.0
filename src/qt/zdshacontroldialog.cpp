...
#include "ui_zdshacontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zdshaTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zdshaTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zDSHA.";
            else if (!mint.isSeedCorrect)
                strReason = "The zDSHA seed used to mint this zDSHA is not the same as currently hold in the wallet";
            else
