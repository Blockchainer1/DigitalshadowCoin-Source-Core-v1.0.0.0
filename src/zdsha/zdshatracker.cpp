...
CzDSHATracker::CzDSHATracker(std::string strWalletFile)
{
...
CzDSHATracker::~CzDSHATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDSHATracker::Init()
{
...
bool CzDSHATracker::Archive(CMintMeta& meta)
{
...
bool CzDSHATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDSHATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDSHATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDSHATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzDSHATracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzDSHATracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzDSHATracker::GetSerialHashes()
{
...
CAmount CzDSHATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDSHATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDSHATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDSHATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDSHATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDSHATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDSHATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDSHATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDSHATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDSHATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDSHATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzDSHAWallet* zDSHAWallet)
{
    bool iszDSHAWalletInitialized = (NULL != zDSHAWallet);
    CMintMeta meta;
    ...
    if (! iszDSHAWalletInitialized)
        zDSHAWallet = new CzDSHAWallet(strWalletFile);
    meta.isSeedCorrect = zDSHAWallet->CheckSeed(dMint);
    if (! iszDSHAWalletInitialized)
        delete zDSHAWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzDSHATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDSHATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDSHATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDSHATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDSHATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDSHATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzDSHAWallet* zDSHAWallet = new CzDSHAWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zDSHAWallet);
        }
        delete zDSHAWallet;
        LogPrint("zero", "%s: added %d dzdsha from DB\n", __func__, listDeterministicDB.size());
    }
