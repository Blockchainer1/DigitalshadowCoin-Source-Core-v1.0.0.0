...
CzDSHAWallet::CzDSHAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdsha seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDSHASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDSHASeed_deprecated()) {
                    LogPrintf("%s: Updated zDSHA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdsha seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdsha wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDSHAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDSHACount(nCountLastUsed);
    else if (!walletdb.ReadZDSHACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDSHAWallet::Lock()
{
    seedMaster = 0;
}

void CzDSHAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDSHAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDSHA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDSHAWallet::LoadMintPoolFromDB()
{
...
void CzDSHAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDSHAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDSHAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdshaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDSHAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDSHA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdshaTracker which also adds to database
    pwalletMain->zdshaTracker->Add(dMint, true);
...
void CzDSHAWallet::SeedToZDSHA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDSHAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDSHAWallet::GenerateDeterministicZDSHA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDSHAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDSHAWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzDSHAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
